...
test('prepare - sharedProcess.on is not a function', () => {
  const error = new TypeError()
  error.message = `sharedProcess.on is not a function`
  error.stack = `TypeError: sharedProcess.on is not a function
    at Module.launchSharedProcess (/test/packages/main-process/src/parts/LaunchSharedProcess/LaunchSharedProcess.js:49:17)
    at async Module.hydrate (/test/packages/main-process/src/parts/App/App.js:102:3)
    at async Module.main (/test/packages/main-process/src/parts/Main/Main.js:16:3)`
  // @ts-ignore
  fs.readFileSync.mockImplementation(() => {
    return `import { MessageChannelMain } from 'electron'
...
`
  ...
  const prettyError = PrettyError.prepare(error)
  expect(prettyError.type).toBe('TypeError')
  expect(prettyError.message).toBe('sharedProcess.on is not a function')
  expect(prettyError.codeFrame).toBe(`  47 |     name: 'shared-process',
  48 |   })
> 49 |   sharedProcess.on('error', handleChildError)
     |                 ^
  50 |   sharedProcess.on('exit', handleChildExit)
  51 |   sharedProcess.on('disconnect', handleChildDisconnect)
  52 |   HandleIpc.handleIpc(sharedProcess)`)
  expect(prettyError.stack)
    .toBe(`    at launchSharedProcess (/test/packages/main-process/src/parts/LaunchSharedProcess/LaunchSharedProcess.js:49:17)
